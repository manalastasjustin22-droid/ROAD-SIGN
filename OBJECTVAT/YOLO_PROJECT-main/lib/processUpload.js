...
// ✅ Register custom font (Ensure fonts/OpenSans-Regular.ttf exists in your project)
...
export async function processUpload({ file, base64 }) {
  ...
  try {
    ...
    // ✅ NO OBJECTS DETECTED CASE
    // If Roboflow returns nothing, we assume the road is clear/safe, or just "No Objects"
    if (!predictions?.length) {
      if (imagePath && fs.existsSync(imagePath)) fs.unlinkSync(imagePath);
      return {
        class_name: "No Objects Detected",
        confidence_score: 0,
        description: "No vehicles, signs, or pedestrians detected.",
        action: "Proceed with normal caution.",
        safety_rule: "Always stay alert even on clear roads.",
    ...
    // Change color to Orange/Yellow for Traffic Alerts (optional, keeps Red if preferred)
    ...
    const cloudinaryRes = await cloudinary.uploader.upload(
      ...
      { folder: "traffic-object-detection" } // Updated folder name
    );

    const annotatedImageUrl = cloudinaryRes.secure_url;
    const confidence = Math.round(Number(top.score) * 100);

    // ✅ AI Suggestion (Traffic Analyst Persona)
    // Note: getAISuggestions now expects `className` not `disease`
    ...
    const logData = {
      id: uuidv4(),
      class_name: top.label, // DB Column: class_name
      confidence_score: confidence,
      description: ai.description,
      action: ai.action,       // DB Column: action (was prescription)
      safety_rule: ai.safety_rule, // DB Column: safety_rule (was mitigation)
      annotated_image_url: annotatedImageUrl,
      predictions: JSON.stringify(predictions),
      created_at,
    };

    // ✅ Insert into DB (Updated Columns)
    await pool.query(
      `INSERT INTO "dbo"."logs"
       (id, class_name, confidence_score, description, action, safety_rule, annotated_image_url, predictions, created_at)
    ...
    // Clear caches
    cache.del("traffic_summary"); // Updated cache key
